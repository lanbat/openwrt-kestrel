...
# The tier name is derived from the conf filename: vpn-bg.conf → "bg".
# All category names are automatically prefixed with the tier name, so
# DNS_CATS="torrentsites" in vpn-bg.conf produces set dns_bg_torrentsites4
# and local file local-dns-bg_torrentsites.txt — no manual prefixing needed.
...
for _conf in "$LISTS_DIR"/vpn-*.conf; do
  ...
  for c in ${DNS_CATS:-};     do
    ALL_DNS_SETS="$ALL_DNS_SETS dns_${_tier}_${c}"
    ALL_LOCAL_FILES="$ALL_LOCAL_FILES local-dns-${_tier}_${c}.txt"
  done
  for c in ${RESOLVE_CATS:-}; do
    ALL_RESOLVE_SETS="$ALL_RESOLVE_SETS resolve_${_tier}_${c}"
    ALL_LOCAL_FILES="$ALL_LOCAL_FILES local-resolve-${_tier}_${c}.txt"
  done
done
...
# ── update-routing-sets (auto-generated from vpn-*.conf on every install) ───────

{
  cat <<'SCRIPT_HEADER'
#!/bin/sh
# Auto-generated by install.sh from /etc/split-routing/vpn-*.conf
# Do not edit — re-run install.sh to regenerate.
# To change remote URLs, edit DNS_URLS_<cat> / RESOLVE_URLS_<cat> in the vpn-*.conf.
# To add or remove categories, edit DNS_CATS / RESOLVE_CATS in vpn-*.conf.
...
dns() {
  ...
  [ -f "$local_file" ] && nftset_from_list "$rule" <"$local_file" >>"$tmp"
  if ! [ -s "$tmp" ]; then
    rm -f "$tmp"
...
resolve() {
  cat=$1; shift
  echo "==> resolve $cat"
  local local_file="/etc/split-routing/local-resolve-${cat}.txt"
  local extra=""
  [ -f "$local_file" ] && extra="url=$local_file"
...
SCRIPT_HEADER
  ...
  _prev_dns_cats=""; _prev_resolve_cats=""
  for _conf in "$LISTS_DIR"/vpn-*.conf; do
    [ -f "$_conf" ] || continue
    # Clear URL vars from the previous conf's categories before sourcing the next one
    for _c in ${_prev_dns_cats:-};     do eval "unset DNS_URLS_${_c}"; done
    for _c in ${_prev_resolve_cats:-}; do eval "unset RESOLVE_URLS_${_c}"; done
    _tier=$(basename "$_conf" .conf | sed 's/^vpn-//')
    ...
    for _c in ${DNS_CATS:-}; do
      eval "_cat_urls=\${DNS_URLS_${_c}:-}"
      printf 'dns %s_%s' "$_tier" "$_c"
      for _u in ${_cat_urls:-}; do printf ' \\\n  %s' "$_u"; done
      printf ' \\\n  2>&1 | tee /tmp/dns-%s_%s.log\n' "$_tier" "$_c"
    done
    for _c in ${RESOLVE_CATS:-}; do
      eval "_cat_urls=\${RESOLVE_URLS_${_c}:-}"
      printf 'resolve %s_%s' "$_tier" "$_c"
      for _u in ${_cat_urls:-}; do printf ' \\\n  %s' "$_u"; done
      printf ' \\\n  2>&1 | tee /tmp/resolve-%s_%s.log\n' "$_tier" "$_c"
    done
    _prev_dns_cats="${DNS_CATS:-}"
    _prev_resolve_cats="${RESOLVE_CATS:-}"
  done
} | write_script /usr/sbin/update-routing-sets
...
# ── local lists ────────────────────────────────────────────────────────────────
# Template files live in $SCRIPT_DIR/<tier>/local-dns-<cat>.txt.
# They are deployed as /etc/split-routing/local-dns-<tier>_<cat>.txt.

for _conf in "$LISTS_DIR"/vpn-*.conf; do
  [ -f "$_conf" ] || continue
  _tier=$(basename "$_conf" .conf | sed 's/^vpn-//')
  unset DNS_CATS RESOLVE_CATS
  . "$_conf"
  for c in ${DNS_CATS:-}; do
    _dst="$LISTS_DIR/local-dns-${_tier}_${c}.txt"
    [ -f "$_dst" ] && continue
    _src="$SCRIPT_DIR/${_tier}/local-dns-${c}.txt"
    if [ -f "$_src" ]; then
      cp "$_src" "$_dst"
    else
      printf '# %s — domains routed via %s VPN\n# One domain per line.\n' "$c" "$_tier" >"$_dst"
    fi
  done
  for c in ${RESOLVE_CATS:-}; do
    _dst="$LISTS_DIR/local-resolve-${_tier}_${c}.txt"
    [ -f "$_dst" ] && continue
    _src="$SCRIPT_DIR/${_tier}/local-resolve-${c}.txt"
    if [ -f "$_src" ]; then
      cp "$_src" "$_dst"
    else
      printf '# %s — domains/IPs routed via %s VPN\n# One domain or IP per line.\n' "$c" "$_tier" >"$_dst"
    fi
  done
done
...

{
  ...
  # Sets for each VPN tier (prefixed with tier name)
  for _conf in "$LISTS_DIR"/vpn-*.conf; do
    [ -f "$_conf" ] || continue
    _tier=$(basename "$_conf" .conf | sed 's/^vpn-//')
    unset VPN_IFACE DNS_CATS RESOLVE_CATS
    ...
    [ -n "${VPN_IFACE:-}" ] || continue
    for c in ${DNS_CATS:-}; do
      printf 'set dns_%s_%s4 { type ipv4_addr; flags dynamic,timeout; timeout %s; }\n' "$_tier" "$c" "$DNS_TIMEOUT"
      printf 'set dns_%s_%s6 { type ipv6_addr; flags dynamic,timeout; timeout %s; }\n' "$_tier" "$c" "$DNS_TIMEOUT"
    done
    for c in ${RESOLVE_CATS:-}; do
      printf 'set resolve_%s_%s4 { type ipv4_addr; flags interval; }\n' "$_tier" "$c"
      printf 'set resolve_%s_%s6 { type ipv6_addr; flags interval; }\n' "$_tier" "$c"
    done
  done
  ...
  for _conf in "$LISTS_DIR"/vpn-*.conf; do
    ...
    [ -n "${VPN_IFACE:-}" ] || continue
    for c in ${DNS_CATS:-}; do
      printf '    ip  daddr @dns_%s_%s4 meta mark set %s\n' "$_tier" "$c" "$FWMARK"
      [ "$ROUTE_IPV6" = yes ] && printf '    ip6 daddr @dns_%s_%s6 meta mark set %s\n' "$_tier" "$c" "$FWMARK"
    done
    for c in ${RESOLVE_CATS:-}; do
      printf '    ip  daddr @resolve_%s_%s4 meta mark set %s\n' "$_tier" "$c" "$FWMARK"
      [ "$ROUTE_IPV6" = yes ] && printf '    ip6 daddr @resolve_%s_%s6 meta mark set %s\n' "$_tier" "$c" "$FWMARK"
    done
  done
  printf '}\n'
} >"$NFTD"
