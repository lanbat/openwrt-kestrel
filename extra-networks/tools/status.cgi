...
for _conf in "${BASE_DIR}"/*-notify.conf; do
    ...
    if [ -n "$_devs" ]; then
        ...
        printf '<table><tr><th style="width:2rem;text-align:center;padding:.45rem .15rem"></th><th>Label</th><th>DNS</th><th>IPv4</th>'
        ...
        printf '%s\n' "$_devs" | while IFS=$(printf '\t') read -r _hn _ip _mac _exp_ts; do
            ...
            printf '<td>%s</td><td class="dim">%s</td><td>%s</td>' \
                "$_label_cell" "$(_html "$_dns")" \
                "$([ "$_ip" = "-" ] && echo "—" || _html "$_ip")"
            [ "$_global_hdr_ip6"  = yes ] && printf '<td class="dim">%s</td>' "${_ipv6:----}"
            printf '<td class="dim">%s</td>' "$_joined"
